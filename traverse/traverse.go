...
func Traverse(entryDir string) {
	...
	fmt.Println("Start to collect filebase: ", entryDir)
...
func worker(jobs chan string, results chan string, wg *sync.WaitGroup, dirWg *sync.WaitGroup) {
	defer wg.Done()
	for job := range jobs {
		if err := collect(job, results, jobs, dirWg); err != nil {
...
func collect(f string, result chan string, queue chan string, dirWg *sync.WaitGroup) error {
	defer dirWg.Done()
